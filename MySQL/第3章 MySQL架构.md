...
​	错误日志记录了MySQL运行过程中遇到的所有严重的错误信息，以及MySQL每次启动和关闭的详细信息。默认是开启的，而且从5.5.7以后无法关闭错误日志。默认的错误日志名称:hostname.err。

​	错误日志所记录的信息是可以通过log-error和log-warnings来定义的，其中log-err是定义是否启用错误日志的功能和错误日志的存储位置，log-warnings是定义是否将警告信息业定义至错误日志中。
